<?php
  class CGraph {
    ...
    ****************************************************************/
    ...
    function LoadGraph($path,$info,$data1) 
    {
      if (($fp = @fopen($path, "r")) !== false) {
        ...
        $this->__LoadGraphDefinitions($content,$info,$data1);
        return true;
      } else return false;
    }
    ...
    function __LoadGraphDefinitions($text,$info,$data1) {
      $text = preg_split("/\r?\n/", $text);
      $data = array();
      $section = '';
...
//      }
    ...
    function __LoadGraphValues($data,$info) {
...
//            $this->graph_title = $value;
          ...
          case 'border-width':
            ...
            break;
          case 'area-height':
            $this->graph_areaheight = (int) $info['height'];
            $this->graph_height = $this->graph_padding['top'] + (int)$info['height'] + $this->graph_padding['bottom'];
            break;
          case 'area-width':
            $this->graph_areawidth = (int) $info['width'];
            $this->graph_width = $this->graph_padding['left'] + (int)$info['width'] + $this->graph_padding['right'];
            break;
          default:
    ...
    function __LoadAxisValues($data,$info) {
      foreach ($data as $name => $value) {
        $name = strtolower($name);
        switch ($name) {
          case 'x-step':
    ...
    function __LoadScaleValues($data,$info) {
      foreach ($data as $name => $value) {
        $name = strtolower($name);
        switch ($name) {
          case 'x-round':
    ...
    function __LoadLegendValues($data,$info) {
      foreach ($data as $name => $value) {
        $name = strtolower($name);
        switch ($name) {
          case 'background-color':
